...
import { saveExtraction, updateStatus } from '../models/extraction/extractionDataService'
...
const start = async (repoId: number, branches: {}, path: string, progLangs: number[]): Promise<string | null> => {
    ...
    try {
        extractionId = await saveExtraction(repoId, branches, path, progLangs);
        ...
        for (const project of projects) {
            ...
            await updateSkillTree(undefined, skillTree.children, projectId, extractionId, true)
        }
        log('Extraction finished.', extractionId)
        updateStatus(extractionId, 'COMPLETED')
        return null
    } catch(err) {
        ...
        log('Extraction failed! - ' + errorMessage, extractionId)
        updateStatus(extractionId, 'FAILED')
        return errorMessage
    }
}
