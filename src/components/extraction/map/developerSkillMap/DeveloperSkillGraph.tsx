...
const DeveloperSkillGraph = (): ReactElement => {
    ...
    function formatTooltip(params: any) {
        if (!params.data.type) {
            return `<table><tr><td><b>Ranking:&nbsp;</b></td><td>${params.data.value}</td></tr></table>`
        } else if (selectedResourceType === 'ALL' || selectedResourceType === params.data.type) {
            return `<table><tr><td><b>Type:&nbsp;</b></td><td>${params.data.type}</td></tr><tr><td><b>Name:&nbsp;</b></td><td>${params.data.name}</td></tr></table>`
        } else {
            return `<table><tr><td><b>Type:&nbsp;</b></td><td>${params.data.type}</td></tr><tr><td><b>Name:&nbsp;</b></td><td>${params.data.name}</td></tr><tr><td><b>Ranking:&nbsp;</b></td><td>${params.data.ranking}</td></tr><tr><td><b>Score:&nbsp;</b></td><td>${params.data.value}</td></tr></table>`
        }
    }
    ...
    const zoom = (inOut: string) => {
        ...
        graph?.setOption(getGraphOption(data, _zoomValue), { notMerge: false, lazyUpdate: true })
    }
    ...
    const createNode = (id: string, type: string, name: string, value: number, ranking: string, category: number, symbolSize: number, symbol: string, itemStyle?: {}): any => {
        return {
            id: id,
            type: type,
            name: name,
    ...
    const transformData = (developerSkillMap: DeveloperSkillMapType[]): any => {
        ...
        for(const rec of developerSkillMap) {
            ...
            if (!developerIds.has(rec.developerId)) {
                ...
                nodes.push(
                    createNode('DEVELOPER'+rec.developerId, 'DEVELOPER', rec.developerName + ' (' + rec.developerEmail + ')', rec.score, rec.ranking, category, getSymbolSize(selectedResourceType, 'DEVELOPER', maxScore, minScore, rec.score), developerSymbol, { color: 'red' } )
                )
            }
            ...
            if (!skillIds.has(rec.skillId)) {
                ...
                const category = [...categories].indexOf(rec.progLang)
                nodes.push(
                    createNode('SKILL'+rec.skillId, 'SKILL', rec.skillName, rec.score, rec.ranking, category, getSymbolSize(selectedResourceType, 'SKILL', maxScore, minScore, rec.score), skillSymbol )
                )
            }
            // adding to links
            links.push({
    ...
    useEffect(() => {
        if (developerSkillMap && developerSkillMap.length > 0) {
            ...
            graph.setOption(getGraphOption(data, 0.2), { notMerge: false, lazyUpdate: true })
                ...
                <>
                    {developerSkillMap &&
                        <>
                            ...
                            <select onChange={selectNode}>
                                {developerSkillMap.filter(rec => rec..type === 'DEVELOPER').map((rec, index) => (
                                    <option value={index} key={index}>{rec.name}</option>
                                ))}
                            </select>
                            <select onChange={selectNode}>
                                {transformData(developerSkillMap).nodes.filter(rec => rec.type === 'SKILL').map((rec, index) => (
                                    <option value={index} key={index}>{rec.name}</option>
                                ))}
                            </select>
                        </>
