    ...
    return (
        <>
            ...
            <i className="bi bi-x-octagon" title='The skill is disabled'></i>
                <span 
                    className={node.children.length > 0 ? 
                               (showChildren ? 'caret caret-down' : 'caret') : 
                               'caret-no-children'}>
                    <input 
                        type='checkbox' 
                        onChange={handleChange} 
                        id={'cb_selected_'+node.id} />
                    <label 
                        className={(node.children.length > 0 ? 'treeLabel pointer' : 'treeLabel') + 
                                   (!node.enabled ? ' disabledSkill' : '')} 
                        onClick={handleClick}>
